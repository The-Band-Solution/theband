defmodule TheBandWeb.IndefinidaTest do
  ...
  describe "o rótulo" do
    test "diz indefinida e o motivo, e o tipo encontrado quando há" do
      assert ConceptLabel.indefinida("type_unknown", "Spike") ==
               "indefinida — tipo Spike sem regra"

      assert ConceptLabel.indefinida("type_absent", nil) == "indefinida — sem tipo na origem"
      assert ConceptLabel.indefinida(nil, nil) == "indefinida — sem promoção registrada"
    end

    test "indefinida NÃO é conceito da base de conhecimento" do
      refute KnowledgeBase.concept?("sro.undefined")
      refute KnowledgeBase.concept?("indefinida")
      ...
      refute Enum.any?(ConceptLabel.conceitos(), fn {_id, rotulo} -> rotulo == "indefinida" end)
    end
  end
  ...
  describe "nas telas" do
    test "a listagem de trabalho nomeia as indefinidas", %{conn: conn} do
      ...
      assert html =~ "indefinida —"
      assert html =~ "tipo Spike sem regra"
    end

...
# ---------------------------------------------------------------------------
# Chave mestra das credenciais — FR-005, FR-005a
#
# Vale para todos os ambientes, de propósito. Não há chave de desenvolvimento
# embutida: uma chave no repositório é uma chave vazada, e um fallback silencioso
# em dev faria a verificação de FR-005a passar por acidente.
#
# A ausência não é tratada aqui — quem recusa o boot é TheBand.Application, para
# que a falha aconteça antes de qualquer supervisor subir.
# ---------------------------------------------------------------------------
master_key =
  case {System.get_env("THE_BAND_MASTER_KEY"), config_env()} do
    {nil, :test} ->
      ...
      Base.encode64(<<0::size(256)>>)

    {value, _} ->
      value
  end

config :the_band, TheBand.Vault,

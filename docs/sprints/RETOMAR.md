...
> dependências a cada `git checkout` — seis vezes num dia. Corrigido no #836, com gate
> que impede a volta.
...
> nem versão na página — `/health`, `/version` e `/api/version` devolvem 404. Confirmar a
> versão no ar exige olhar o Dokploy. É a lacuna que a regra nova do PO e do Design fecha,
> e ela nasceu **junto** com esta release, não antes dela.
...
O guarda do `docs.yml` confere `CNAME`, o `index.html` da raiz **e** `/docs/index.html` —
o último porque `/docs/` deixou de ser gerado e um deploy que o apagasse publicaria com
...
| **#839** | o item de investigação do estado divergente da issue | squash |
...
tem cenário de ataque no formato que o QA transforma em teste, e quatro foram **medidos
com teste**, não deduzidos.
...
3. **H7 e H8 — sem release nenhum.** O Dokploy implanta `latest`, e as ações do CI usam
   tag mutável. Os únicos que se consertam sem código nem deploy;
...
`docs/backlog/investigar-estado-divergente-da-issue.md`, PR #839. Dois casos reais, com
URL nos dois lados, e **são defeitos diferentes com o mesmo sintoma**:

- **apagada** na origem, aberta na tela. `collected_issues` **tem**
  `no_longer_observed_at`, e o `grep` por quem o marca encontra só designações e
  etiquetas — nada, aparentemente, marca a issue em si;
...
**Comece pelo banco de produção**, e não pelo código: o `state` e o `collected_at` da
linha contra a data do fechamento no GitHub distinguem as quatro hipóteses.
...
US7 foi construído fora de ordem** — a US7 não tem tarefa alguma, e a US8 também tem
código em produção sem tarefa.
...
E o segredo **não se guarda**: hash irreversível, mostrado uma vez, sem recuperação. A
tela avisa **antes** de gerar.

---

## Duas armadilhas que este dia ensinou, e as duas são de processo

**`git add -A` numa árvore compartilhada com subagentes.** O commit `5d02075` varreu
**1.450 linhas** de dois outros papéis para dentro de um PR de documentação que dizia
mudar três arquivos. Refeito. **Usar caminhos explícitos** quando há agentes em paralelo.

**Anunciar número de PR sem ter aberto o PR.** Aconteceu duas vezes: a branch foi
empurrada e o PR não. Conferir com `gh pr list --head <branch>` antes de citar número.

E uma que virou gate: **dos oito PRs abertos em 2026-09-09, zero declararam o tipo de
merge** — `gh pr create --body` substitui o template inteiro, e a exigência desaparece em
silêncio. O #834 fechou isso: PR sem declaração agora reprova no CI.

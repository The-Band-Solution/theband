...
|---|---|---|
| [O relatório da pessoa e da equipe](relatorio-da-pessoa-e-da-equipe.md) | pedido de 2026-09-25: gerar, em **HTML, PDF ou Markdown para IA**, o retrato do que a tela da pessoa e a da equipe mostram. **É dado de pessoa num arquivo que sai da plataforma**, a mesma superfície do MCP: o veredito da tela vale na geração (inclusive o do perfil, H2-R), nada que a tela esconde entra, a geração fica registrada, e o texto de terceiro sai como texto. A D1 foi decidida: **HTML primeiro**. O **Markdown para IA** tem a superfície do MCP: texto de terceiro só dentro de código, nenhum link ou imagem montado de dado | **D1 decidida, D2–D6 a decidir** — exige avaliação do `security` antes do plano e protótipo antes do código |
| [As quatro decisões da tela de tokens](decisoes-da-tela-de-tokens.md) | as seis perguntas abertas do protótipo, respondidas em 2026-09-23. **Duas mudam o que está implementado**: *sem expiração* passa a ser oferecido — revertendo a regra que dizia *máximo do qual se abre mão não é máximo* — e revogar passa a gravar o motivo. O protótipo foi republicado e **aprovado em 2026-09-23**, sem o aviso por e-mail | **alta — as três mudanças de tela estão aprovadas e por implementar**; a régua do `PROMPT.md` ganha itens antes do código |
| [O aviso de vencimento só alcança quem abre a tela](aviso-de-vencimento-do-token.md) | a Q6 do protótipo, **adiada em 2026-09-23**. A linha de 14 dias existe e é lida no render; o que não existe é qualquer alcance fora da tela — quem não abre `/api-tokens` descobre o vencimento pela recusa da integração. E-mail é feature própria: medido no mesmo dia, a plataforma **não envia e-mail nenhum**. O passo barato é a linha na tela de operação | **baixa — adiado por decisão**, e a medida que o repriorizaria ainda não existe |
| [O plano de correções da API](plano-de-correcoes-da-api.md) | **as sete pendências que a v0.9.0 deixou**, em ordem, com a razão de cada posição. Duas já corrigidas e só esperando release; duas sem item até hoje (poda do registro de leitura, e a tela que o leia); uma que depende da feature 062. Mais a lacuna que não tem código: revisão independente não ocorreu | **alta — é o índice das outras**, escrito em 2026-09-23 |
| [O log não distingue as três causas de recusa](sc004-o-log-nao-distingue-as-tres-recusas.md) | **SC-004 da 061, o único critério que reprovou por defeito.** Token inexistente, revogado e expirado produzem `motivo=credencial_recusada` no log — as três colapsam numa. `Token.estado/2` já calcula a distinção, e o `_` do `else` em `autenticar/1` a descarta. A resposta ao cliente deve continuar idêntica (SC-003); o que falta é o log interno | média — achado em 2026-09-23 na aceitação da 061 |
| [Dois testes reprovam de vez em quando](teste-instavel-do-fluxo.md) | `fluxo_na_tela_test.exs:205` devolveu `{:error, :invalid}` ao montar a LiveView. **Mesma semente, resultados diferentes**: 2314 passaram, depois 2313 de 2314, depois 2314 de novo com a semente que reprovou. Gate que reprova por acaso treina quem lê a reexecutar até passar — e a próxima reprovação real vira mais uma instabilidade | **dois** casos agora: `fluxo_na_tela_test.exs:205` e `eventos_de_acesso_test.exs:134`. Os dois passam isolados e reprovam sob carga — não é teste mal escrito, é a suíte competindo por CPU e relógio | **baixa** — rebaixada em 2026-09-23: seis execuções da suíte inteira, 2361/2365 idêntico nas seis, e nenhum dos dois reapareceu. Os dois testes ganharam diagnóstico para que a próxima ocorrência diga o que aconteceu |
| [`Done` é alegação, não aceite](done-e-alegacao-nao-aceite.md) | cartão sai de `Done` — **193 vezes, em 185 issues**, e 58% por outra pessoa que não a que pôs. Das 185, **69 não voltaram**: contá-las no gráfico é contar entrega recusada. A rede já proibia materializar `sro.accepted_deliverable` a partir da coluna (`sro.rule03`); estes são os números. Decide o que a 066 faz quando o cartão volta | **alta — entra no critério de fim da 066**, medido em 2026-09-16 |
| [O booleano `active` não diz quem desativou](booleano-active-nao-diz-quem-desativou.md) | duas tabelas guardam "está ligado?" como booleano onde **nove** guardam data e autor. Perde quem desligou, quando, e se já foi religado — e numa credencial de ferramenta isso é pergunta de segurança. **Não é defeito**: nada está errado hoje, e trocar não recupera o passado | baixa — o gatilho é a próxima mexida numa das duas, achado em 2026-09-18 |
| [O evento de coluna não diz o quadro](o-evento-de-coluna-nao-diz-o-quadro.md) | `ProjectV2ItemStatusChangedEvent` guarda o **nome da coluna** e não o quadro. Das 377 issues Done do quadro 43, **235 estão em dois quadros**, e a chegada a `Done` que encontramos pode ser do outro. A origem oferece o campo `project` — introspecção de 2026-09-16 colada no item; a consulta nunca o pediu. Conserto é barato, a recoleta que ele exige é que custa | **alta — a medida de entrega do Conecta Fapes depende dela**, achado em 2026-09-16 |
| [A timeline vem truncada, e a origem diz que não](timeline-truncada-na-origem.md) | **RESOLVIDO em 2026-09-16** — corrigido no [#923](https://github.com/The-Band-Solution/theband/pull/923) e a base recolhida no [#924](https://github.com/The-Band-Solution/theband/pull/924). Fica pelo registro do defeito: dentro de `issues(first:50)` a origem devolve **10 itens** de timeline por issue e declara `totalCount` igual a 10, com `hasNextPage: false`; por `issue(number:)` devolve os 19 reais. **282 entregas do quadro 43 não têm data** por causa disso, e o alcance é toda a base. A guarda existente nunca dispara porque confia no total que a origem mente | **fechado** |
| [O link para a issue na origem](link-para-a-issue-na-origem.md) | quem olha a lista quer ir à issue no GitHub sem passar pelo detalhe; hoje o único link externo vive no detalhe, e a `url` da issue **não é coletada** — o endereço é composto do repositório mais o número, e quebra se o repositório for renomeado | média — pedido de quem usa, 2026-09-14 |
| [O recorte por sprint](o-recorte-por-sprint.md) | *atrasou · nunca realizada · concluída no sprint* — três estados que não existem. O *Sprint 41* do quadro 43 fechou em 14/09 com **100% de carry-over**, e nenhuma tela diz isso. **Filtrar só quem tem sprint apagaria a entrega**: 23 dos 27 entregues de uma pessoa não têm sprint | **alta — depois da 066**, pedido de 2026-09-14 |
| [Arquivados e duplicatas](arquivados-e-duplicatas.md) | arquivar card **não** afeta medida (a plataforma conta por issue + responsável, e a API devolveu zero arquivados); o que afeta são **41 issues abertas em títulos repetidos**. Duplicata é `stateReason`, nunca título igual; e `project_items.no_longer_observed_at` existe e nunca é preenchida | média — pequeno e independente, 2026-09-14 |
| [A complexidade dos cards](complexidade-nos-cards.md) | o quadro 43 tem `Story Points` preenchido em **229 de 1 194**, coletado cru e nunca interpretado. A resposta a *"atua em demandas complexas?"* é **distribuição, nunca média** — e **965 não estimados** escritos | média — pedido com destino escrito pelo autor, 2026-09-14 |
| [Paridade entre o `compose.yaml` e o Dokploy](paridade-entre-compose-e-dokploy.md) | o que o compose declara deve existir no servidor. O vão é o MinIO — e subi-lo no mesmo VPS dá paridade de ambiente e **não dá backup**, porque o incêndio que leva o banco leva o backup junto. A produção num segundo host **já está decidida** (2026-09-12, [item do MinIO](minio-como-destino-do-ensaio-de-backup.md)); o que resta é se há um MinIO de **ensaio** no VPS, com esse nome, e onde fica o segundo host | **bloqueada no que resta decidir** — proposta de 2026-09-13, estreitada em 2026-09-14 |
| [As 17 specs sem issue — a decisão de não preencher](specs-antigas-sem-issue.md) | 390 tarefas sem issue, e a caixa do `tasks.md` diz que 255 estão abertas — **a T001 da spec 001, "gerar o projeto Phoenix", está aberta**. Criar issue a partir desse dado produziria erro, não ausência | **decidido em 2026-09-13: não preencher; declarar a fronteira na spec 042** |
...
| [A aba *Flow per person* na tela da equipe](tela-da-equipe-fluxo-por-pessoa.md) | a terceira aba de `/teams/:id`: o fluxo de cada membro na mesma janela, sem ranking e sem média por pessoa. Protótipo atestado como aprovado em 2026-09-08, com endereço e prompt; requisitos escritos em 2026-09-10 — FR-085 a FR-131 da spec [060](../../specs/060-tela-da-equipe/spec.md). **Nove decisões pendentes**, três travando | **média — não decomponível ainda**: a aprovação do protótipo não tem registro, e Q20/Q21/Q25 travam código e aceitação |

...
- [x] T002 Configurar locales e domínios do catálogo
  - **Pronta quando**: T001 concluída; contrato `contracts/catalogo-de-mensagens.md` escrito (está)
  - **Descrição**: `config/config.exs` ganha `config :gettext, :default_locale, "en"` (a config de runtime — a do backend é compile-time; corrigido na implementação com razão no contrato) e `config :the_band, TheBandWeb.Gettext, allowed_locales: ["en", "pt"]`; criar `priv/gettext/pt/LC_MESSAGES/` com `errors.po` e `sistema.po` vazios válidos e os `.pot` dos dois domínios — estrutura do data-model.md. Nenhuma frase migra aqui
...
- [x] T003 Verificador de literais, começando pela violação
...
- [x] T004 O gate "mensagens no catálogo"
...
- [x] T005 [US1] As mensagens da 045 migram sem mudar um byte
...
- [x] T006 [US1] Os flashes de erro dos LiveViews migram para errors
...
- [x] T007 [US2] As confirmações e avisos migram para sistema
...
- [x] T008 [US2] As pendências de tela, medidas e nomeadas
...
- [x] T009 [US3] O relatório de lacunas
...
- [x] T010 [US3] A troca de idioma provada, com pt nascendo pelas recusas
...
- [x] T011 Gates verdes e PR no padrão

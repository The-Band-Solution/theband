...
| **R1.2** | o select de expiração oferece **90 days — the declared maximum**, **30 days**, **7 days**, e **não** oferece "no expiration" | as três opções existem, 90 dias é a primeira, e não há quarta opção | há opção de "sem expiração" no formulário (a menos que Q1 seja decidida como (b)) |
| **R1.3** | caixa tracejada **"No expiration is not offered here"** citando `api.access.token_lifetime` e dizendo que tokens antigos continuam lendo *no expiration* na lista | a caixa existe, com o nome do limiar e a frase dos tokens antigos | a opção simplesmente não está lá, sem explicação |
...
| **R2.19** | caixa **"what this list does not answer"**: só **quando**, nunca **o que** a integração leu, com a lacuna declarada (Q6) | a caixa existe e nomeia a lacuna | não há nada dizendo isso, e a coluna de último uso passa por auditoria |
...
| # | o item | aprovado quando | reprovado quando |
|---|---|---|---|
| **R5.1** | **seis recusas**, cada uma escrita como o que a pessoa esperaria encontrar: autoatendimento · editar/estender · ver o que a integração leu · reativar · dar escopo ao token · rever o valor | as seis existem, cada uma com a expectativa e a resposta | falta qualquer uma, ou a tela só omite o controle |
...
| **R6.5** | **nenhum número no código**: 90 dias, 30 dias e 14 dias vêm de `api.access.thresholds` | `grep` por `90`/`14` no módulo da tela e da fronteira não acha o prazo | qualquer prazo como constante de módulo |

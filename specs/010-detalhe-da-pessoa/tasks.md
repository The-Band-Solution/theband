...
- [ ] T002 [US2] Contar os papéis cadastrados
  ...
  - **Descrição**: `EO.count_roles/1`, lendo `eo_organizational_roles`. Existe para a tela
    **explicar** a não promoção com base no **dado**, e não em texto fixo — R4.
    ...
    Três casos, e o terceiro é impossível hoje e vai existir:
    ...
    Texto fixo daria a explicação errada com convicção no dia em que alguém cadastrar papel, e ninguém
    notaria: a frase continuaria plausível
...
- [ ] T003 [US3] Contar designações e autorias separadas
  ...
  - **Feita quando**: para uma pessoa com 12 designações e 7 autorias, devolvem **12** e **7**;
    designação marcada como não mais observada **não** é contada
  - **Teste**: `test/the_band/work_items/person_work_test.exs` — as duas contagens, e a asserção de
    que designação ausente não conta. **Nenhum teste soma as duas**, porque nada no código deveria
    poder somá-las
...
- [ ] T008 [US2] Dizer o que a plataforma não promoveu
  ...
  - **Descrição**: a seção de equipes na página, com um **componente privado do LiveView** para a
    ...
    **Privado do LiveView**, e não em `TheBandWeb.UI`: há três usos **na mesma tela**, e o critério de
    promoção é o segundo consumidor **fora** dela — R5.

    A explicação da não promoção vem do **dado** (T002), com os três casos. E **nenhum texto** usa a
...
- [ ] T009 [US3] Mostrar o trabalho sem somar
  ...
  - **Feita quando**: os dois números aparecem separados e a soma não aparece; os repositórios trazem
    a marca de derivado com a evidência; pessoa sem trabalho tem as duas ausências nomeadas; a lista
    pagina
  - **Teste**: o mesmo arquivo de T006 — e o teste que mais importa **procura o número proibido**:
    com 12 e 7, `refute html =~ ">19<"`. É o mesmo teste que na feature 006 achou um defeito real que
    eu havia introduzido
...
**17 de 17 requisitos com tarefa.** SC-001 a SC-012 verificados por V1 a V9 do
